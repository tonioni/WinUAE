...
static int atonce_hdd_resolve(void)
{
	...
	for (int nr = 0; nr < MAX_FILESYSTEM_UNITS; nr++) {
		...
		int found_rdb = 0;
		for (int b = 0; b < 16; b++) {
			if (hdf_read(hfd, blk, (uae_u64)b * 512, 512, &err) != 512)
				break;
		...
		for (int guard = 0; part != 0xffffffff && guard < 64; guard++) {
			if (hdf_read(hfd, blk, (uae_u64)part * 512, 512, &err) != 512)
				break;
			...
			pname[pl] = 0;
			uae_u8 mbr[512];
			int rd = hdf_read(hfd, mbr, part_off, 512, &err);
...
static int atonce_disk_bypass(uint16_t port)
{
	...
	case 0x03: {   // write sectors
		...
		for (int s = 0; s < count; s++) {
			...
			if (func == 0x02) {
				if (hdf_read(atonce_hfd, sb, sp, 512, &err) != 512) { ok = 0; break; }
				for (int i = 0; i < 512; i++)
					mem_writeb_phys(buf + s * 512 + i, sb[i]);
			} else {
				for (int i = 0; i < 512; i++)
					sb[i] = mem_readb_phys(buf + s * 512 + i);
				if (hdf_write(atonce_hfd, sb, sp, 512, &err) != 512) { ok = 0; break; }
			}
			done++;
		}
