...
drive_sounds_fetcher="${source_dir}/tools/fetch-drive-sounds.sh"
drive_sounds_checker="${source_dir}/tools/require-drive-sounds.sh"
...
require_file "${drive_sounds_fetcher}" "drive sound fetcher"
require_file "${drive_sounds_checker}" "drive sound manifest check"
...
require_match 'WINUAE_UNIX_BUNDLE_DRIVE_SOUNDS' "${cmake}" "drive sound bundle option"
require_match 'download\.abime\.net/winuae/stuff/drive_sounds\.zip' "${cmake}" "drive sound archive URL"
require_match '2efbe66d090c50f5b58e1248a1cab5c1886c7d203bd2f1d26d1ec382b3ffb088' "${cmake}" "drive sound archive SHA-256"
require_match 'DESTINATION "\$\{WINUAE_UNIX_INSTALL_PLUGINDIR_RELATIVE\}/floppysounds"' "${cmake}" "drive sound install rule"
...
require_match 'WINUAE_FLOPPYBRIDGE_PLUGIN_FILE' "${cmake}" "prebuilt FloppyBridge plugin option"
require_match 'winuae_unix_floppybridge_package_check' "${cmake}" "FloppyBridge package requirement"
require_match 'install\(FILES "\$\{WINUAE_FLOPPYBRIDGE_PLUGIN_PATH\}"' "${cmake}" "FloppyBridge plugin install rule"
...
require_match 'add_dependencies\(package winuae_unix_floppybridge_plugin\)' "${cmake}" "package FloppyBridge dependency"
...
require_match 'WINUAE_DRIVE_SOUNDS_FETCH' "${readme}" "drive sound fetch documentation"
...
require_match 'package does not contain drive sound sample' "${deb_builder}" "Debian drive sound contents check"
require_match 'drive_click_\*\.wav' "${drive_sounds_checker}" "drive sound set validation"
require_match 'package does not contain FloppyBridge\.so' "${deb_builder}" "Debian FloppyBridge contents check"
